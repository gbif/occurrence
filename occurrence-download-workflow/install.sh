...
curl -H 'Authorization: token $TOKEN' -H 'Accept: application/vnd.github.v3.raw' -O -L https://api.github.com/repos/gbif/gbif-configuration/contents/occurrence-download-workflow/$VERSION/profiles.xml

echo "Assembling workflow jar for $ENV"
mvn --settings profiles.xml -P$P -DskipTests clean package assembly:single
