    ...
    stage('Maven build: Main project (Java 17)') {
      ...
      steps {
        configFileProvider([
          ...
          sh 'mvn -s ${MAVEN_SETTINGS} clean deploy -T 1C -Dparallel=classes -DuseUnlimitedThreads=true -Pgbif-dev -U -Djetty.port=${JETTY_PORT} -DargLine="--add-opens=java.base/java.lang.invoke=ALL-UNNAMED" -Dappkeys.testfile=${APPKEYS_TESTFILE} -B -pl !occurrence-table-build-trino'
        }
      }
    }
