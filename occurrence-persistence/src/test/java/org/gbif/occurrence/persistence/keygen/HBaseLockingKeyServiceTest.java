...
public class HBaseLockingKeyServiceTest {
  ...
  public void testLockWriteDie() throws IOException {
    ...
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_LOCK_COLUMN), 0, lock1);
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_KEY_COLUMN), Bytes.toBytes(2));
    lookupTable.put(put);
    ...
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_LOCK_COLUMN), 0, lock2);
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_KEY_COLUMN), Bytes.toBytes(3));
    lookupTable.put(put);
  ...
  public void testConflictingIds() throws IOException {
    ...
    Put put = new Put(lookupKey1);
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_STATUS_COLUMN), Bytes.toBytes("ALLOCATED"));
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_KEY_COLUMN), Bytes.toBytes(1));
    lookupTable.put(put);
  ...
  public void testStaleLock() throws IOException {
    ...
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_LOCK_COLUMN), 0, lock);
    HTableInterface lookupTable = tablePool.getTable(LOOKUP_TABLE);
    lookupTable.put(put);
    lookupTable.flushCommits();
    lookupTable.close();
  ...
  public void testValidLockBecomesStale() throws IOException {
    ...
    Put put = new Put(lookupKey);
    put.add(Bytes.toBytes(TableConstants.LOOKUP_COLUMN_FAMILY),
      Bytes.toBytes(TableConstants.LOOKUP_LOCK_COLUMN), ts, lock);
    HTableInterface lookupTable = tablePool.getTable(LOOKUP_TABLE);
    lookupTable.put(put);
    lookupTable.flushCommits();
    lookupTable.close();
