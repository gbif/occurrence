 ...
 private final TableBackfillConfiguration configuration;
  ...
  public String insertOverwriteMultimediaTable() {
    return String.format("INSERT OVERWRITE TABLE %1$s_multimedia\n"
                         ...
                         + "LATERAL VIEW EXPLODE(json_tuple(occ_mm.ext_multimedia, 'type', 'format', 'identifier', 'references', 'title', 'description', 'source', 'audience', mm_record.created, 'creator', 'contributor', 'publisher', 'license', 'rightsHolder')) AS mm_record)", configuration.getTableName());
  }
