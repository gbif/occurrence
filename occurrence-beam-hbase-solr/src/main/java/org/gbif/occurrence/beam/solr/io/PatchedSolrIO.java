...
/**
 * Patched: experimenting on batch size, longer timeouts, commitWith and retries
 *
 *
...
public class PatchedSolrIO {
  ...
  public static Read read() {
    ...
    return new AutoValue_PatchedSolrIO_Read.Builder().setBatchSize(1000).setQuery("*:*").build();
  }
  ...
  public static Write write() {
    ...
    return new AutoValue_PatchedSolrIO_Write.Builder().setMaxBatchSize(1000).build();
  }

  private PatchedSolrIO() {}
  ...
  public abstract static class ConnectionConfiguration implements Serializable {
    ...
    public static ConnectionConfiguration create(String zkHost) {
      ...
      return new AutoValue_PatchedSolrIO_ConnectionConfiguration.Builder().setZkHost(zkHost).build();
    }
    ...
    private HttpClient createHttpClient() {
      ...
      HttpClientUtil.setConnectionTimeout(client, 600000); // 10 mins
      HttpClientUtil.setSoTimeout(client, 600000); // 10 mins
  ...
  abstract static class ReplicaInfo implements Serializable {
    ...
    static ReplicaInfo create(Replica replica) {
      return new AutoValue_PatchedSolrIO_ReplicaInfo(
  ...
  public abstract static class Write extends PTransform<PCollection<SolrInputDocument>, PDone> {
    ...
    static class WriteFn extends DoFn<SolrInputDocument, Void> {
      ...
      private void flushBatch() throws IOException {
        ...
        // patch: a quick hack with a naive retry mechanism
        int maxAttempts = 10;
        long sleepMs = 5000;
        try {
          for (int attempt=1; attempt <= 10; attempt++ ) {
            try {
              ...
              batch.forEach(d -> updateRequest.add(d, 300000)); // 5 mins
              ...
              if (attempt > maxAttempts) {
                ...
                System.err.println("Error writing to SOLR [" + attempt + " ] of [" + maxAttempts + "] attempts");
                // swallow and retry
                try {
