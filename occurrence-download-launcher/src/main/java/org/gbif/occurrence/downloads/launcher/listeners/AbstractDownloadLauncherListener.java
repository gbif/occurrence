...
public abstract class AbstractDownloadLauncherListener
  ...
  private final DownloadLauncher occurrenceDownloadLauncherService;
  ...
  protected AbstractDownloadLauncherListener(
      DownloadLauncher occurrenceDownloadLauncherService,
  ...
  public void handleMessage(DownloadLauncherMessage downloadsMessage) {
    try {
      log.info("Received {}message {}", getMessagePrefix(), downloadsMessage);
      ...
      if (!getDownloadUpdaterService(downloadsMessage).isStatusFinished(downloadKey)) {
        ...
        if (jobStatus == JobStatus.RUNNING) {
          ...
          log.info("Locking the thread until {}download job is finished", getMessagePrefix());
          ...
          log.error("Failed to process {}message: {}", getMessagePrefix(), downloadsMessage);
