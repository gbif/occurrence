...
public class DownloadEsClient implements Closeable {
  ...
  @SneakyThrows
  private QueryBuilder searchQuery(Predicate predicate) {
  ...
  public long getRecordCount(org.gbif.api.model.occurrence.predicate.Predicate requestPredicate) {
    ...
    CountResponse response = esClient.count(new CountRequest().indices(esIndex).query(searchQuery(predicate)),
