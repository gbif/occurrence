...
public class HiveSQLValidator {
    private static final Pattern INVALID_TOKENS =  Pattern.compile(".*(INSERT_INTO|ALTER|DROP|UPDATE|SHOW|CREATE|DELETE|USE|TRUNCATE|LOAD|UNION|VIEW|WINDOW|LOCK|EXPLAIN|ANALYZE|JOIN|DISABLE|GRANT|PRIVILEGE|PRIV|SET|TRANSFORM).*");
    ...
    public HiveSQLValidator(HiveConf hiveConf) {
      this.hiveConf = hiveConf;
    }
  
    public HiveSQLValidator() {
    ...
    private static RuntimeException newParseError(ASTNode node) {
      return new IllegalArgumentException(String.format("Invalid select statement contains a %s in line %s  and column %s",
                                                        node.getToken().getText().replaceFirst("TOK_", ""),
    ...
    public void validateQuery(String query) {
      try {
        ...
        Preconditions.checkArgument(nodeQl.getChildren() != null
          && nodeQl.getChildren().size() == 2
          && ((ASTNode)nodeQl.getChildren().get(0)).getToken().getType() == HiveParser.TOK_QUERY, "Query must have only one statement");
