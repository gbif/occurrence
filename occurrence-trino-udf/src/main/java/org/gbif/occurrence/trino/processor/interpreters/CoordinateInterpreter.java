...
import org.gbif.occurrence.trino.processor.clients.GeocodeWsClient;
import org.gbif.occurrence.trino.processor.result.CoordinateResult;
import org.gbif.occurrence.trino.processor.util.CountryMaps;
import org.gbif.occurrence.trino.processor.util.Wgs84Projection;
...
public class CoordinateInterpreter {
  ...
  private static final Map<List<OccurrenceIssue>, BiFunction<Double, Double, LatLng>> TRANSFORMS =
      new LinkedHashMap<>();
  ...
  static {
    ...
    TRANSFORMS.put(
        Collections.singletonList(OccurrenceIssue.PRESUMED_NEGATED_LATITUDE),
        (lat, lng) -> new LatLng(-1 * lat, lng));
    TRANSFORMS.put(
        Collections.singletonList(OccurrenceIssue.PRESUMED_NEGATED_LONGITUDE),
        (lat, lng) -> new LatLng(lat, -1 * lng));
    TRANSFORMS.put(
        Arrays.asList(
            OccurrenceIssue.PRESUMED_NEGATED_LATITUDE, OccurrenceIssue.PRESUMED_NEGATED_LONGITUDE),
        (lat, lng) -> new LatLng(-1 * lat, -1 * lng));
    TRANSFORMS.put(
        Collections.singletonList(OccurrenceIssue.PRESUMED_SWAPPED_COORDINATE),
        (lat, lng) -> new LatLng(lng, lat));
  }
  ...
  /**
   * Attempts to convert the given lat and long into Doubles, and the given country string into an
   * ISO country code.
   *
   ...
   *     known errors were encountered in the interpretation (e.g. lat/lng reversed). Or all fields
   *     set to null if latitude or longitude are null
   */
  public OccurrenceParseResult<CoordinateResult> interpretCoordinate(
      String latitude, String longitude, String datum, final Country country) {
   ...
   */
  public OccurrenceParseResult<CoordinateResult> interpretCoordinate(
      String latLon, String datum, final Country country) {
    return verifyLatLon(CoordinateParseUtils.parseVerbatimCoordinates(latLon), datum, country);
  }

  private OccurrenceParseResult<CoordinateResult> verifyLatLon(
      final OccurrenceParseResult<LatLng> parsedLatLon, final String datum, final Country country) {
    ...
    OccurrenceParseResult<LatLng> projectedLatLon =
        Wgs84Projection.reproject(
            parsedLatLon.getPayload().getLat(), parsedLatLon.getPayload().getLng(), datum);
    ...
    for (Map.Entry<List<OccurrenceIssue>, BiFunction<Double, Double, LatLng>> geospatialTransform :
        TRANSFORMS.entrySet()) {
    ...
    if (interpretedLatLon == null) {
      ...
      interpretedLatLon =
          OccurrenceParseResult.fail(coord, OccurrenceIssue.COUNTRY_COORDINATE_MISMATCH);
    }
    ...
    if (interpretedLatLon.getPayload() == null) {
      ...
      LOG.warn(
          "Supposed coordinate interpretation success produced no latlng {}", interpretedLatLon);
      return OccurrenceParseResult.fail(issues);
    }

    return OccurrenceParseResult.success(
        interpretedLatLon.getConfidence(),
        new CoordinateResult(interpretedLatLon.getPayload(), finalCountry),
        issues);
  }
  ...
  private static boolean isAntarctica(Double latitude, @Nullable Country country) {
    ...
    return (country == null || country == Country.ANTARCTICA)
        && (latitude >= -90 && latitude < ANTARCTICA_LATITUDE);
  }

  /**
   * It's theoretically possible that the webservice could respond with more than one country,
   * though it's not known under what conditions that might happen.
   *
   * <p>It happens when we are within 100m of a border, then both countries are returned.
   */
  private List<Country> getCountryForLatLng(LatLng coord) {
    ...
    try {
      Collection<Location> response =
          geocodeClient.get(
              coord.getLat(), coord.getLng(), null, null, Collections.singletonList("Political"));
      ...
      if (response != null) {
        ...
        // If no country is returned from the geocode, add Antarctica if we're sufficiently far
        // south
